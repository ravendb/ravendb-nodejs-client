...
describe('DocumentSession', () => {
  ...
  beforeEach((done: MochaDone) => {
    ...
    const indexMap: string = "from doc in docs select new {name = doc.name,key = doc.key,doc_id = doc.key+\"_\"+doc.name}";
    const indexSort: IndexDefinition = new IndexDefinition('Testing_Sort', indexMap, null, {
     ...
     executor.execute(new PutIndexesCommand(indexSort))
     .then(() => {
        session = store.openSession();
          return Promise.all([
            ...
            session.store(session.create({name: 'testing_order',key: 92, product_id: 'products/108'}, 'order'), 'orders/105'),
            session.store(session.create({name: 'withNesting', product: {name: 'testing_order',key: 4, order: null}}, 'company'), 'company/1')
      ])
      .then(() => done());
      });
  });
  ...
  describe('Index checking', () => {
    ...
    it('should make query with fetch terms', (done: MochaDone) => {
      session = store.openSession();
      session.query(null,'Testing_Sort', null, true).whereBetweenOrEqual('key', 2, 4).search('doc_id').get().then((results) => {
        for(let result of results) {
